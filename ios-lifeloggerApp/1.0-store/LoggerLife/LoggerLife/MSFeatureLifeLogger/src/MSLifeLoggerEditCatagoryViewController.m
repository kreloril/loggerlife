...
-(void)onSelectActionOptions:(id)sender {
    ...
    [self.navigationController pushViewController:[MSFeatureLifeLogger BuildScreenforStack:optionStack withNavController:self.navigationController screen:MSLifeLoggerScreenCatagoryOptions] animated:YES];
}
...
-(void)save:(id)sender {
    ...
    if (!self.isNew) {
        [[MSFeatureLifeLogger instance] updateLifeloggerCatagory:self.activeCatagory];
    } else {
        [[MSFeatureLifeLogger instance] insertLifeLoggerCatagory:self.activeCatagory];
    }
    [self close:nil];
}
...
-(void)performDeleteMove {
    ...
    MSLifeLoggerCatagory* trashCat = [[MSFeatureLifeLogger instance] findCatagory:CATAGORY_DEFAULT_TRASH];
    MSLifeLoggerCatagory* currentCat = [MSFeatureLifeLogger instance].activeCatagory;
    
    if (trashCat && currentCat) {
        [[MSFeatureLifeLogger instance] moveAllMessagesFromCatagory:currentCat to:trashCat];
        [[MSFeatureLifeLogger instance] removeCatagory:currentCat];
    }
    
    [[MSFeatureLifeLogger instance] changeCatagory:[[MSFeatureLifeLogger instance] findCatagory:CATAGORY_DEFAULT_DAILY]];
    [self close:nil];
}
@end
