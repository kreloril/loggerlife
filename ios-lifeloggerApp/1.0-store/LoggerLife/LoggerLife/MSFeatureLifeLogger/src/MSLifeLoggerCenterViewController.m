...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    NSDictionary* entry = self.objects[section];
    NSString* entryDate = [MSFeatureLifeLogger headerFormattedTime:  entry.allKeys[0]];
...
-(MSLifeLoggerLogEntry*)logEntryForIndex:(NSIndexPath*)indexPath {
    ...
    NSDictionary* entry = self.objects[indexPath.section];
    NSArray* all = entry.allValues[0];
...
-(void)onActionDeleteAllMessages:(id)sender {
    ...
    [controller addAction:[UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (istrash) {
            [[MSFeatureLifeLogger instance] removeAllMessagesFromCatagory:LIFELOGGER_CURRENT_CATAGORY];
        } else {
            [[MSFeatureLifeLogger instance] moveAllMessagesFromCatagory:LIFELOGGER_CURRENT_CATAGORY to: [[MSFeatureLifeLogger instance] findCatagory:CATAGORY_DEFAULT_TRASH]];
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [[MSFeatureLifeLogger instance] changeCatagory:LIFELOGGER_CURRENT_CATAGORY];
            self.objects = [MSFeatureLifeLogger instance].arrayOfActiveCatagoryObjects;
            [self.tableView reloadData];
        });
    }]];
...
-(void)onActionAddButton:(id)sender {
    ...
    if ([MSFeatureLifeLogger instance].activeCatagory == nil) {
    ...
    [self.navigationController pushViewController:[MSFeatureLifeLogger BuildScreenforStack:stack2 withNavController:self.navigationController screen:MSLifeLoggerScreenAddEditEvent] animated:YES];
}
