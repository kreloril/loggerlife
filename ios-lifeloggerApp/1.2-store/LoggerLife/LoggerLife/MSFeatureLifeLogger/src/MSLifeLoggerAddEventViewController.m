...
- (void)viewDidLoad {
 ...
 //   [self.entry setObject: [NSString stringWithFormat:@"%f",[NSDate date].timeIntervalSinceReferenceDate] key:@"timestamp"];
    ...
    if (!self.entry) {
        ...
        NSString *catid = [MSFeatureLifeLogger instance].featureDataBase.activeCatagory.catid;
        
        [self.entry setObject:catid key:@"ownerid"];
...
-(void)processEditOptions {
    ...
    [controller addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[MSFeatureLifeLogger instance].featureDataBase updateLogEntry:self.entry withReload:true] ;
    ...
    [controller addAction:[UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[MSFeatureLifeLogger instance].featureDataBase removeLogEntry:self.entry];
        [self.navigationController popViewControllerAnimated:YES];
         });
    }]];
    ...
    [controller addAction:[UIAlertAction actionWithTitle:@"Set Finished" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (self.entry.entryState == MSLifeLoggerEntryStateFlagged) {
         ...
         [[MSFeatureLifeLogger instance].featureDataBase updateLogEntry:self.entry withReload:false];
    ...
    [controller addAction:[UIAlertAction actionWithTitle:@"Move to Catagory.." style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self.navigationController pushViewController:[MSFeatureScreenBuilder BuildScreenforStack:stack2 withNavController:self.navigationController screen:MSLifeLoggerScreenChangeCatagory] animated:YES];
...
-(void)onRightButton:(id)sender {
    ...
    [[MSFeatureLifeLogger instance].featureDataBase insertLogEntry:self.entry];
    [self.navigationController popViewControllerAnimated:YES];
}
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        cell.textLabel.text = [MSFeatureDateFormatting formattedTimeStamp:self.entry.timestamp];
        NSLog(@"%@ %@",[MSFeatureDateFormatting formattedTimeStamp:self.entry.timestamp],self.entry.timestamp);
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:@"TextViewCell"];
        ...
        if (self.oneTimeSetCellText) {
            ...
            [self.loggingTextView becomeFirstResponder];
        }
    } else {
