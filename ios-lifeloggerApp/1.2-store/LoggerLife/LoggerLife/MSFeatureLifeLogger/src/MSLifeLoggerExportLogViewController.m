...
- (void)viewDidLoad {
    ...
    if (LIFELOGGER_CURRENT_DATABASE.activeCatagory) {
           [self setTitle:[NSString stringWithFormat:@"%@",LIFELOGGER_CURRENT_DATABASE.activeCatagory.name]];
            self.objects = LIFELOGGER_CURRENT_DATABASE.arrayOfActiveCatagoryObjects;
       } else {
           self.objects = nil;
       }
    NSLog(@"%@",self.objects);
  ...
  //  [self.exportText appendString:[self.objects description]];
    ...
    for ( int i = 0; i < self.objects.count ; i++) {
        ...
        [self.exportText appendString:[MSFeatureDateFormatting formattedTime:objectEntry.allKeys[i]]];
        [self.exportText appendString:@"\n\n"];
